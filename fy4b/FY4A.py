...
class FY4A(CFG):
    ...
    def gen_new_data(self, data: ndarray, ori_lat: ndarray, ori_lon: ndarray, new_lat: ndarray, new_lon: ndarray,
        ...
        new_data = np.where(new_data == -888.0, np.nan, new_data)
        new_data = np.where(new_data >= 65530, np.nan, new_data)
        return new_data
    ...
    def read_LMI(self, file_path: str) -> xr.Dataset:
        ...
        ds = xr.open_dataset(file_path)
        ds = fy4Utils.rename_ds(ds, 'LAT', 'lat')
        ds = fy4Utils.rename_ds(ds, 'LON', 'lon')
        return ds
    ...
    def get_data_from_data_name(self, file_path: str, data_name: str, new_lon: ndarray, new_lat: ndarray,
                                method: str = 'nearest', sv_path: str = '') -> Dataset:
        ds = self.read_FY4A(file_path)
        data_to_modify = ds[data_name]
        # 修改数据格式
        data_to_modify = data_to_modify.astype('float32')
        # 将修改后的数据重新写入数据集
        ds[data_name] = data_to_modify
        return ds
