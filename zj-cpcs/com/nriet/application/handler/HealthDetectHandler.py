...
class HealthDetectHandler(RequestHandler):
    """
    基本操作类，之后的类可以直接继承
    """
    ...
    def doing(self, *args, **kwargs):
        ...
        logging.info('%s is exists ? %s' % (file_name, os.path.exists(file_name)))
        if os.path.exists(file_name):
            with open(file_name, mode='r', encoding='utf-8') as file:
                is_healthy = file.readline()
                logging.info('current environment value %s ' % (is_healthy))
                if is_healthy:
                    if is_healthy.endswith('yes'):
                        return 200, "Server status's healthy."
                    elif is_healthy.endswith('no'):
                        return 503, "Server status's unhealthy, cannot handle any request!"
