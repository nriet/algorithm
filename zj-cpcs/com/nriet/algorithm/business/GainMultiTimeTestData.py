...
class GainMultiTimeTestData(InputDataComponent):
    ...
    def execute(self, params):
        ...
        if len(self.forecastTime.split(",")) == 1:
            forecast_time_list = self.forecastTime.split(",")
            moniStartTime = DateUtils.getForwradTime(self.forecastTime, self.timeType, int(self.startTime))
            moniEndTime = DateUtils.getForwradTime(self.forecastTime, self.timeType, int(self.endTime))
        else:
            ...
            moniStartTime = DateUtils.getForwradTime(forecastStartTime, self.timeType, int(self.startTime))
            moniEndTime = DateUtils.getForwradTime(forecastEndTime, self.timeType, int(self.endTime))
        print(moniStartTime,moniEndTime)
        # 获取站点监测数据
        moni_data_list = self.get_station_data(self.timeType, moniStartTime, moniEndTime, self.element, self.areaCode)
        ...
        if len(self.forecastTime.split(",")) == 1:
            moni_time_list = DateUtils.get_time_list([moniStartTime, moniEndTime], self.timeType)
            jy_data = np.full((len(moni_time_list), len(data_source_list)), np.nan)

            for j, data_source in enumerate(data_source_list):
                if data_source in ["CFSV2", "CPSV3"]:
                    if self.methodType in ["PS", "PC", "ACC", "SYMBOLRATE"]:
                        staticType = "JP"
                    if self.methodType in ["SCC", "SRMSE"]:
                        staticType = "SK"
                    mode_data = self.get_single_data(data_source, self.element, staticType, self.reportTimeType,
                                                     self.timeType, self.forecastTime, moniStartTime, moniEndTime,
                                                     "115,125,25,34")
                    if mode_data is None:
                        continue
                    # 模式插值到站点数据
                    m2s_data = mode_data.interp(lat=staInfoData.sel(space="lat"), lon=staInfoData.sel(space="lon"))
                for i, moni_time in enumerate(moni_time_list):
                    moni_data_avg = moni_data[i]
                    m2s_data_avg = m2s_data[i]
                    ...
                    jy_data[i][j] = jy_val
            print(jy_data)
            jy_data = xr.DataArray(jy_data, coords=[moni_time_list, data_source_list], dims=["time", "model"])
            jy_data = xr.where(np.isnan(jy_data), 999999, jy_data)
        else:
            jy_data = np.full((len(forecast_time_list), len(data_source_list)), np.nan)
            for i, forecat_time in enumerate(forecast_time_list):
                tmpStartTime = DateUtils.getForwradTime(forecat_time,self.timeType,int(self.startTime))
                tmpEndTime = DateUtils.getForwradTime(forecat_time,self.timeType,int(self.endTime))
                tmp_time_list = DateUtils.get_time_list([tmpStartTime,tmpEndTime],self.timeType)
                ...
                for j, data_source in enumerate(data_source_list):
                    if data_source in ["CFSV2", "CPSV3", "NCC", "CFS2", "JMA3", "UKMO5", "EC5", "CPSV3MON"]:
                        ...
                        if self.methodType in ["SCC", "SRMSE"]:
                            staticType = "SK"
                        mode_data = self.get_single_data(data_source, self.element, staticType, self.reportTimeType,
                                                         self.timeType, forecat_time, tmpStartTime, tmpEndTime,
                                                         "115,125,25,34")
                        if mode_data is None:
                            continue
                        # 模式插值到站点数据
                    ...
                    jy_data[i][j] = jy_val
            print(jy_data)
            jy_data = xr.DataArray(jy_data,coords=[forecast_time_list,data_source_list],dims=["time","model"])
            jy_data = xr.where(np.isnan(jy_data),999999,jy_data)
        ...
        if len(self.forecastTime.split(",")) == 1:
            ...
            res_data_dict["xAxisData"] = [DateUtils.time_format_en(tt, self.timeType, "-") for tt in moni_time_list]
            res_data_dict["xAxisDataCh"] = [DateUtils.time_format_ch(tt, self.timeType) for tt in moni_time_list]
        else:
            ...
            res_data_dict["xAxisData"] = [DateUtils.time_format_en(tt, self.timeType, "-") for tt in forecast_time_list]
            res_data_dict["xAxisDataCh"] = [DateUtils.time_format_ch(tt, self.timeType) for tt in forecast_time_list]
        if self.timeType == "day":
            subTitle = subTitle + " 检验时段：未来" + self.startTime + "-" + self.endTime + "天"
        if self.timeType == "mon":
            subTitle = subTitle + " 检验时段：未来" + self.startTime + "-" + self.endTime + "月"
...
if __name__ == "__main__":
    try:
        ...
        #                "dataSource": "CFSV2,CPSV3",
        #                "element": "AVGT",
        #                "methodType": "PS"}
        gmttd = GainMultiTimeTestData()
