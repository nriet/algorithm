...
# @Author: Shiys
# @File: PolarVortexIntensityIndex.py
import xarray as xr
import numpy as np
import math
...
class PolarVortexIntensityIndex(BusComponent):
        ...
        if len(shape) == 3:
            ...
            for i,tt in enumerate(time_list):
                ...
                if np.isnan(tmp_data).all():
                    ...
                    pv_list.append(self.cal_pv_qd(tmp_data, tmp_value, 1, 4))

        pv_index = xr.DataArray(pv_list, coords=[self.inputData.time], dims=['time'])
        output_data = {
            "outputData": pv_index
        }
        self.output_data = output_data

    def cal_pv_qd(self,data, value, n1, n2):
        """
        ...
        for i in range(1,ny):
            for j in range(0,nx):
                ...
                if len(np.where(tdata <= value)[0]) >= n2:
                    jw_area_data[i, j] = (value-data[i,j])*area[i]
        # 计算满足条件的各点的面积之和
...
if __name__ == '__main__':
    ...
    gg = PolarVortexIntensityIndex({"R":"6371","gridDistance":"2.5"},[{"outputData":nc_data}])
    gg.execute()