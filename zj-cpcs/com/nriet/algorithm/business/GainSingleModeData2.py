...
class GainSingleModeData2(InputDataComponent):

    def __init__(self, sub_local_params, algorithm_iuput_datas):
        # 加载数据源配置
        dataSourceConfigPath = PathConfig.CPCS_ROOT_PATH + 'com/nriet/config/dataSourceConfig2.json'
    ...
    def execute(self):
        ...
        if self.dataSource in ["CPSV3","CFSV2"]:
            mode_data = self.get_single_data(self.dataSource, self.element, self.eleType, str(self.reportTimeType.lower()),
                                             str(self.timeType.lower()), self.forecastTime, self.startTime, self.endTime,
                                             "115,125,25,34")
        else:
            if self.dataSource in ["CFSV2MON"]:
                mode_data = self.get_single_dm_data(self.dataSource, self.element, self.eleType, str(self.reportTimeType.lower()),
                                                 str(self.timeType.lower()), self.forecastTime, self.startTime, self.endTime,
                                                 "115,125,25,34")
            elif self.dataSource in ["NCC","UKMO5","JMA3","CFS2"]:
                mode_data = self.get_single_mode_data(self.dataSource, self.element, self.eleType, self.forecastTime, self.startTime,
                                                      self.endTime, "115,123,27,32")
            else:
                mode_data = self.get_single_mode_data2(self.dataSource, self.element, self.forecastTime, self.startTime,
                                                      self.endTime, "115,123,27,32")
        glon = np.linspace(117, 123, 61)
        glat = np.linspace(27, 32, 51)
        if self.dataSource == "ZJPPC":
            mode_data = mode_data / 50 * 100
        ...
        outputData = {}
        outputData["outputData"] = mode_data
    ...
    def get_single_mode_data(self, data_source, element, eleType, forecastTime, startTime, endTime, region):
    ...
    def get_single_mode_data2(self,data_source, element, forecastTime, startTime, endTime, region):
        dataCfg = self.datasource_config[data_source.upper()+"_"+element.upper()]
        dataInputPath = dataCfg["dataInputPath"]
        var = dataCfg["var"]
