...
class WpIntensity(BusComponent):
    ...
    #     self.output_data = None
    """
    此处筛选28℃海温数据，小于28的全部设置为缺测，大于等于28设置为1
    """

    def GridScreen(self, data):
        return xr.where(data >= 28, 1, np.nan)
    ...
    def Wpsh_Intensity(self, data, R, gridDistance):
        ...
        temp = xr.where(data >= 28, data, np.nan)
        ScreenGrid = self.GridScreen(data)
        Intensity = []
        for i in range(len(lats)):
            Intensity.append(np.nansum(ScreenGrid[i, :].values * area[i] * (temp[i, :].values - 28)))
    ...
    def execute(self):
        ...
        # 监测数据，时间*纬度*经度
        if len(shape) == 3:
            wpsh_Intensity = [self.Wpsh_Intensity(self.inputData[i, :, :], self.R, self.gridDistance) for i in
                              range(shape[0])]
