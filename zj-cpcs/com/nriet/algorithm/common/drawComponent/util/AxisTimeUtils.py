...
def get_month_abbr(month,regex='%m'):
    mon = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]
    mon_abbr = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC']
    return mon_abbr[mon.index(month[-2:])]

def get_day_month(day):
    day_str = str(day)
    return day_str[-2:]+ get_month_abbr(day_str[4:6])

def get_day_month_year(day):
    day_str = str(day)
    return get_day_month(day_str) + day_str[:4] #DDMMYYYY


def get_month_year(day):
    day_str = str(day)
    return  get_month_abbr(day_str) + day_str[:4]
