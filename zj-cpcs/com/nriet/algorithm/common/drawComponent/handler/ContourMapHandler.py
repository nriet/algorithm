...
class ContourMapHandler(CommonHandler):
    ...
    def __init__(self, workstation, plot=None, input_data=None, params_dict=None, layer=None, tmp_service=None):
        ...
        if layer.get('contour_line_color', None):
            params_dict['cnMonoLineColor'] = True
        ...
        if tmp_service:
            params_dict['sfXArray'] = tmp_service.lon
            params_dict['sfYArray'] = tmp_service.lat
    ...
    def draw(self):
        ...
        # # 是否有色板配置文件
        # cn_params_dict = {
        #     "cnLevels": self.intervals
        #     , "cnFillColors": np.array(self.colors)
        #     , 'sfXArray': self.lon
        #     , 'sfYArray': self.lat
        # }
