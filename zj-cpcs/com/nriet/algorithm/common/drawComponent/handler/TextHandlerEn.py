...
def add_legend_unit(im, draw, nboxes, unit, params_dict=None):
    start_time = time.time()
    if im is None or draw is None:
        return
    if params_dict is None:
    ...
    lb_legend_width, lb_legend_height = draw.textsize('Legend', font_lb_legend)
    ...
    if nboxes > 14:
        ...
        draw.text(((lb_width - lb_legend_width) / 2 + outer_left_padding,
                   img_height - (lb_height - inner_top_padding + outer_buttom_padding) - (14 - nboxes) * 5), 'Legend',
                  font=font_lb_legend,
                  fill='black')
        draw.text(((lb_width - lb_unit_width) / 2 + outer_left_padding,
