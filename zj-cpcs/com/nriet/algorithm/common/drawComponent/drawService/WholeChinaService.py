...
class WholeChinaService(CommonService):
    ...
    def draw(self):
        ...
        for i, layer in enumerate(layers):
            ...
            if layer_type == 'contourMap':
                if layer.get('map_masking_areas'):
                    china_params_dict['south_sea']['contourMap']["mpMaskAreaSpecifiers"] = layer.get('map_masking_areas')
        ...
        if first_layer.get('cnLine'): # 绘制等值线时不添加图例
            if first_layer.get('cnLine') == 'True' or first_layer.get('cnLine'):
                pass
            else:
            ...
            if first_layer['layer_type'] in ['contourMap','polyMarker']:
                label_bar_handler = LabelBarHandler(workstation, map_plot,
                                                    params_dict=china_params_dict.get('label_bar'), layer=first_layer,
                                                    tmp_service=self)
                label_bar_handler.draw()
        ...
        if first_layer.get('cnLine'): # 绘制等值线时不添加单位
            ...
            if first_layer['layer_type'] in ['contourMap','polyMarker'] :
                TextHandler.add_legend_unit(im, draw, nboxes=len(colors), unit=unit,
                                            params_dict=china_params_dict['legend_unit'],
                                            legends=first_layer.get('ch_legend_labels'),isNaN=first_layer.get('isNaN'))
        # 11.叠加中国中文掩膜
        # 新绘图修改
        ...
        if int(save_to_nfsshare_switch) or self.request_dict.get("saveToNfsshare"):
            ...
            if self.request_dict.get("output_img_type_eps") and int(self.request_dict.get("output_img_type_eps")) == 1:
                ...
                fig.close()
                print(output_img_path + output_img_name + '.eps')
    ...
    def build_contourMap_params(self, layer):
        ...
        if layer.get('cnSmoothingOn') == "False":
            contour_params_dict['cnSmoothingOn'] = False
        ...
        if layer.get('cnLine'):
            if layer.get('cnLine')=='True' or layer.get('cnLine'):
                ...
                if layer.get('intervals', None):
                    ...
                    contour_params_dict['cnExplicitLineLabelsOn'] = True
                    lineLabels = []
                    ...
                    contour_params_dict['cnLineLabelStrings'] = lineLabels
                    if layer.get('contour_line_labels', None):
                        contour_params_dict['cnLineLabelStrings'] = list(layer.get('contour_line_labels'))
                        contour_params_dict['cnLineLabelInterval'] = 1
                        contour_params_dict['cnLineLabelPlacementMode'] = 'constant'
                        contour_params_dict['cnLineDashSegLenF'] = 0.08

                    contour_params_dict['cnLineLabelFontHeightF'] = 0.008
