...
class DrawRegionsHandler_MODEL_SMALL():
        ...
        try:
            ...
            for i, layer in enumerate(layers):
                ...
                if layer_type == "contourMap":  # 色斑图层
                    ...
                    # 插值
                    # glon = np.linspace(lon[0], lon[-1], len(lon) * 10)
                    # glat = np.linspace(lat[0], lat[-1], len(lat) * 10)
                    # smooth_array = layer_data.interp(lat=glat, lon=glon)
                    # 剪裁数据
                ...
                if layer_type == "polyMarker":  # 散点图层
                    ...
                    if layer.get('marker_size'):
                        ...
                        if regionType == "3":
                            marker_size = areaConfig.get('marker_size', 600)
                        else:
            ...
            # 如果是浙江省，叠加的地理信息不同
            if regionType == "1":
                self.shpInfo = ["province_boundary", "city_boundary", "city_label"]
            ...
            #     bar = fig.colorbar(c_bar, cbar_ax)
            ...
            #     # test = bar.ax.xaxis.get_ticklabels()
            ...
            if c_bar:
                if c_bar_style == 1:
                    coef = (endLon - startLon) / (endLat - startLat)
                    # num = str(3 * coef) + "%"
                    # sizeNum = str(coef * 3.5) + "%"
                    # bar = m.colorbar(c_bar, location="bottom", pad=num, size=sizeNum)
                    bar = plt.colorbar(c_bar, pad = 2 * coef / 100, shrink = 1, orientation="horizontal")
                    bar.set_ticks(levs_map)
                    if self.eleType != "TCC":
                        if self.extendColor:
                            levs_map[0] = ''
                    ...
                    # for label in [bar.ax.xaxis.get_ticklabels()[0], bar.ax.xaxis.get_ticklabels()[-1]]:
                    #     label.set_visible(False)
                    bar.ax.tick_params(labelsize=22, tick1On=False)
