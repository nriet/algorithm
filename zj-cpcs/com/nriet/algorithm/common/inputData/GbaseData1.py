...
from com.nriet.config.ResponseCodeAndMsgEum import CUSTOM_ERROR_CODE
...
class GbaseData1(InputDataComponent):
    ...
    def execute(self):
        ...
        if self.eleType.find("SK") != -1:
            ...
            # 封装返回的结果数据
            output_data.append(res_data_mean)
        if self.eleType.find("LTM") != -1:
            ...
            # 封装返回的结果数据
            output_data.append(res_data_ltm)
        # output_data.append({"outputData": self.stationInfoData})
        self.output_data = {"outputData": output_data}
    ...
    def query_mean_data(self, timeType, startTime, endTime, dataSource, elements, areaCode, stationStr, statisticType):
        ...
        # 按区域查询实况数据的sql模板
        sql_tmplate = "SELECT a.stationId, a.want_time D_DATETIME, %s(a.val) val FROM ( SELECT t3.V01301 stationId, t3.D_DATETIME, t3.%s val,%s FROM CIPAS_STATION_INFO t4, CIPAS_AREA_INFO t1, CIPAS_AREA_STATION t2, %s t3 WHERE t1.D_AREACODE = t2.D_AREACODE AND t2.stationId = t3.V01301 AND t2.stationId = t4.stationId AND t4.stationType = 0 AND %s AND t3.D_DATETIME BETWEEN '%s' AND '%s' ) a WHERE a.val < 99999 GROUP BY a.want_time, a.stationId"
        # 按站点查询实况数据的sql模板
        if stationStr:
            sql_tmplate = "SELECT a.stationId, a.want_time D_DATETIME, %s(a.val) val FROM ( SELECT t3.V01301 stationId, t3.D_DATETIME, t3.%s val,%s FROM %s t3 WHERE 1=1 AND %s AND t3.D_DATETIME BETWEEN '%s' AND '%s' ) a WHERE a.val < 99999 GROUP BY a.want_time, a.stationId"
        # 替换sql模板里的占位符(统计方法,表字段,各尺度的时间分组，表名，区域站点)，生成完整的SQL语句
        sql_str = sql_tmplate % (statisticType, tableField, wantTimeStr, tableName, staConditionStr, tempStartTime, tempEndTime)
    ...
    def query_ltm_data(self, timeType, startTime, endTime, dataSource, elements, areaCode, stationStr, statisticType, ltm):
        ...
        if timeType != "year":
            ...
            if startXh > endXh:
                sql_str = sql_str + "and (" + indexFie1d + " >= " + str(startXh) + " OR " + indexFie1d + "<=" + str(endXh)+ ")"
            else:
...
if __name__ == '__main__':
    ...
    sub_local_params = {
        ...
        'statisticType': 'AVG',
        # 'areaCode': 'CH',
        'stationStr': '59945,59951,50136,50137,50246,50247,59948,59954',
        'eleType': 'SK,LTM',
        'timeType': 'day',
        'ltm': '1991-2020',
        'timeRanges': [20180101, 20180503]
    }
    gid = GbaseData1(sub_local_params, None)
    gid.execute()
